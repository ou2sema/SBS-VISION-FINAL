...
export function downloadDevisPDF(data: DevisData): void {
  ...
  const cleanNum = getSafeQuoteNumber(data.quoteNumber);
  doc.save(`Devis-SBS-VISION-${cleanNum}.pdf`);
}

function getSafeQuoteNumber(quoteNumber: string): string {
  return quoteNumber.trim().replace(/[^a-zA-Z0-9-_]/g, '_') || 'devis';
}

/** Uploads the generated PDF and best-effort stores its metadata with the source quote. */
export async function uploadDevisPDF(
  quoteId: string,
  data: DevisData
): Promise<{ downloadUrl: string; metadataSynced: boolean }> {
  const cleanNum = getSafeQuoteNumber(data.quoteNumber);
  const pdfBytes = generateDevisPDF(data).output('arraybuffer') as ArrayBuffer;

  if (isSupabaseConfigured && supabase) {
    const storagePath = `quotes/${cleanNum}.pdf`;
    const { error: uploadError } = await supabase.storage
      .from(supabaseQuotesBucket)
    ...
    try {
      await updateDoc(doc(getFirebaseFirestore(), 'quoteRequests', quoteId), {
        quoteNumber: data.quoteNumber,
        devis: data,
        pdf: {
          storageProvider: 'supabase',
          storageBucket: supabaseQuotesBucket,
          storagePath,
      ...
      return { downloadUrl: publicUrlData.publicUrl, metadataSynced: true };
    } catch (metadataError) {
      console.warn('PDF uploaded to Supabase, but Firestore metadata was not updated:', metadataError);
      return { downloadUrl: publicUrlData.publicUrl, metadataSynced: false };
    }
  }

  const storageRef = ref(getFirebaseStorage(), `quotes/${cleanNum}.pdf`);

  await uploadBytes(storageRef, pdfBytes, {
    contentType: 'application/pdf',
    contentDisposition: `attachment; filename="Devis-SBS-VISION-${cleanNum}.pdf"`,
  });
